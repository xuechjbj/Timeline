...
GLuint createShaderProgram(const char *vertexShader, const char *fregShader)
{
	GLuint program = glCreateProgram();
	GLuint vertex = glCreateShader(GL_VERTEX_SHADER);
	GLuint fragmt = glCreateShader(GL_FRAGMENT_SHADER);

	std::string vertexSrc = readTextFile(vertexShader);
	std::string fragmtSrc = readTextFile(fregShader);
...
GLuint loadTexture(const std::string &imgFile)
{
	int texWidth, texHeight, comp;
	unsigned char *pixelData = stbi_load(imgFile.c_str(), &texWidth, &texHeight, &comp, 0);
