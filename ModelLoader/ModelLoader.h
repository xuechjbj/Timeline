...
struct boneInfo {
	...
	boneInfo():boneOffset(0.0), finalTrans(0.0)
	{
...
public:
	...
	void boneTransform(float secs, vector<glm::mat4>& transforms, int anim, float& anTime);
...
private:
	...
	void readNodeHierarchy(float animTime, const aiNode* pNode, const glm::mat4& parentTrans);
	void loadBones(size_t meshInd, const aiMesh* m, vector<vBoneData>& bones, sMesh smash);
	size_t getNumBones();

	GLuint loadTexure(string file);
